...
int main()
{
    ...
    ES.Subscribe("Test", [](int variable) {                        //Subribe to an evenement "Test"
        std::cout << "Number is " << variable << std::endl;        // When the evenement will be called this function will be execute with the parameter data
    }); 
    ...
    ListenerID soustraction_id = ES.Subscribe("Event 3", [](int a, int b) { return a - b; }); // This function will return a int
    ...
    std::cout << result[soustraction_id] << std::endl;        // Print 9 - 3 so 6 
